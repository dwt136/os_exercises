...
```
1. 使用基于连续内存的内存分配算法与机制，分配4K作为页目录表
2. 初始化页，建立页表，将0xC0000000~0xF8000000映射到0x00000000~0x38000000，将0x00000000~0x00100000映射到0x00000000~0x00100000
3. 将CR0的第31位置1，启动页机制；将CR3置为页目录起始地址
4. 更新GDT
5. 取消0x00000000~0x00100000到0x00000000~0x00100000的映射
...
> 因为在内核态发生了不允许出现的的中断（中断号2012011363 mod 37 == 18），根据trap/trap.c里中断处理程序中的
